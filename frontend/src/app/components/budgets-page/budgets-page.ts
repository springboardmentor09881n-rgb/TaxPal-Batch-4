    ...
    <div class="space-y-6 max-w-6xl mx-auto">
      ...
      <div class="flex justify-end">
        <div class="bg-white border border-slate-200 rounded-xl px-6 py-4 flex items-center justify-between w-64 shadow-sm">
          ...
          <span class="text-sm font-bold text-slate-800">Good</span>
        </div>
      </div>
      ...
      <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
          <h2 class="text-lg font-bold text-slate-800">Create New Budget</h2>
        ...
        <form (ngSubmit)="saveBudget()" class="p-6">
          ...
          <div class="flex justify-end gap-3 mt-4">
            <button type="button" (click)="resetForm()" class="px-5 py-2 rounded-lg text-sm font-semibold text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 transition-colors">
              Cancel
            </button>
            <button type="submit" [disabled]="!formData.category || !formData.budget_amount || !formData.month" class="px-5 py-2 rounded-lg text-sm font-semibold text-white bg-blue-500 hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                  ...
                  <td class="px-6 py-4 text-right">
                    <div class="flex items-center justify-end gap-2">
                      <button (click)="editBudget(budget)" class="text-blue-500 hover:text-blue-700 transition-colors p-1">
                        Edit
                      </button>
                      <button (click)="deleteBudget(budget._id || budget.id!)" class="text-red-500 hover:text-red-700 transition-colors p-1">
                        Delete
                      </button>
                    </div>
                  </td>
