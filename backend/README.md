...
### 6. Reports & Export Services (`/api/reports`)
- **`POST /api/reports/preview`**: Compute live financial summary metrics & table breakdowns (Private).
  - *Body*: `{ reportType, period, startDate?, endDate? }`
- **`POST /api/reports/generate`**: Generate and save report entry in history (Private).
  - *Body*: `{ name?, reportType, period, startDate?, endDate?, format: 'PDF' | 'CSV' }`
- **`GET /api/reports`**: Fetch all saved report records for authenticated user (Private).
- **`GET /api/reports/:id`**: Fetch a single report record by ID (Private).
- **`DELETE /api/reports/:id`**: Delete a saved report record (Private).
- **`POST /api/reports/export/pdf`** / **`GET /api/reports/export/pdf`**: Export and download summary report as PDF (Private).
- **`POST /api/reports/export/csv`** / **`GET /api/reports/export/csv`**: Export and download summary report as CSV (Private).
- **`GET /api/reports/:id/download`**: Download previously saved report by ID (Private).
