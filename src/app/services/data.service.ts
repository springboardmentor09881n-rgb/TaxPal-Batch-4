...
export interface Report {
  id: string;
  userId: string;
  reportType: string; // "Income Statement" | "Tax Summary" | "Expense Report"
  period: string; // "Current Month" | "Last Month" | "Year-to-Date"
  format: 'PDF' | 'CSV';
...
export class DataService {
  ...
  private initDatabase(): void {
    // Check if users database exists, if not seed Indian demo user
    ...
    if (sessionUser) {
      ...
      this.loadUserData(JSON.parse(sessionUser).id);
    }
  }
  ...
  public signup(signupData: any): boolean {
    const users = this.getData<any>('tp_users');
    ...
    const newUser = {
      ...
      country: 'India', // Automatically default to India as requested
      state: signupData.state || 'Maharashtra',
      incomeBracket: signupData.incomeBracket
    };

    users.push(newUser);
    this.saveData('tp_users', users);

    // Seed new user defaults
