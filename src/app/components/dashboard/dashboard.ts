...
import { DataService, Transaction, Budget, Category } from '../../services/data.service';
        ...
        <div class="lg:col-span-8 bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
          ...
          <div class="h-64 w-full">
            <svg class="w-full h-full" viewBox="0 0 600 220" preserveAspectRatio="none">
              ...
              @for (bar of monthlyBars(); track bar.label; let i = $index) {
                <g>
                  <!-- Income Bar -->
                  <rect 
                    [attr.x]="65 + i * 85" 
            ...
            @if (categoryTotals().length > 0) {
              ...
              <div class="relative h-40 w-40 mb-6">
                <svg class="h-full w-full -rotate-90" viewBox="0 0 36 36">
                  ...
                  @for (slice of donutSlices(); track slice.name; let idx = $index) {
                    <circle 
                      cx="18" 
                      cy="18" 
                      r="15.9155" 
              ...
              <div class="w-full space-y-2 text-xs">
                @for (cat of categoryTotals().slice(0, 5); track cat.name) {
...
export class Dashboard {
  ...
  protected categoryTotals = computed<CategoryTotal[]>(() => {
    ...
    return Object.keys(totalsMap).map(catName => {
